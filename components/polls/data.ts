import { ElectionType, StateColors, StateElectionData, StateStats } from './types';

export const electionData: Record<string, StateElectionData> = {
  uttarpradesh: {
    stateName: 'Uttar Pradesh',
    availableElections: [
      {
        name: 'Uttar Pradesh Assembly General Election 2017',
        type: ElectionType.Assembly,
        surveyDate: '11-03-2017',
        estimatedColumn: 'Estimates on 09-03-2017 after calculations',
        actualColumn: 'Actual seats received on 11-03-2017',
        rulingParty: 'BJP',
        mergeColumns: ['District'],
        constituencyModalConfig: {
          constituencyColumn: 'Assembly',
          districtColumn: 'District',
          predictedWinnerColumn: 'Party Name',
          actualWinnerColumn: 'Party Name',
          predictedVotesColumn: 'Estimated Votes (09-03-2017)',
          actualVotesColumn: 'Actual Votes (11-03-2017)',
        },
      },
    ],
  },
  andhrapradesh: {
    stateName: 'Andhra Pradesh',
    availableElections: [
      {
        name: 'Andhra Pradesh Assembly Election 2024',
        type: ElectionType.Assembly,
        estimatedColumn: 'Expected Seats',
        actualColumn: 'Actual Seats',
        rulingParty: 'AP-ALLIANCE',
        partyNameColumns: ['Party Name', 'Expected Winner (SGED)', 'Actual Winner 2024'],
        constituencyModalConfig: {
          constituencyColumn: 'Constituency Name',
          predictedWinnerColumn: 'Expected Winner (SGED)',
          actualWinnerColumn: 'Actual Winner 2024',
          predictedMarginColumn: 'SGED Winning Margin',
          actualMarginColumn: 'Actual Margin 2024',
        },
      },
      {
        name: 'Andhra Pradesh Lok Sabha Election 2024',
        type: ElectionType.LokSabha,
        estimatedColumn: 'Expected Total Seats / Predicted Seats',
        actualColumn: 'Actual Results 2024',
        rulingParty: 'AP-ALLIANCE',
        partyNameColumns: ['Party Name', 'Winning Party Predicted', 'Winning Party Actual'],
        constituencyModalConfig: {
          constituencyColumn: 'Constituency Name',
          predictedWinnerColumn: 'Winning Party Predicted',
          actualWinnerColumn: 'Winning Party Actual',
          predictedMarginColumn: 'Win Margin Predicted',
          actualMarginColumn: 'Win Margin Actual',
        },
      },
    ],
  },
  bihar: {
    stateName: 'Bihar',
    availableElections: [
      {
        name: 'Bihar Legislative Assembly Election 2025',
        type: ElectionType.Assembly,
        estimatedColumn: 'Estimated Seats Prediction',
        actualColumn: 'Actual Seats After Results on 14-11-2025',
        mergeColumns: [
          'Alliance',
          'District',
          'Expected Total Seats',
          'Actual Total Seats After Results 14-11-2025',
          'After Adjustment With Variation By PPA',
          'Differences',
        ],
        rulingParty: 'BJP',
        partyNameColumns: [
          'Party Name',
          'Expected Winning Party (By PPA on 11-11-25)',
          'Actual Winner (after results on 14-11-25)',
          'Runner-Up (By PPA on 11-11-25)',
          'Actual Runner-Up (after results on 14-11-25)',
        ],
        constituencyModalConfig: {
          constituencyColumn: 'Constituency',
          districtColumn: 'District',
          predictedWinnerColumn: 'Expected Winning Party (By PPA on 11-11-25)',
          actualWinnerColumn: 'Actual Winner (after results on 14-11-25)',
          predictedRunnerUpColumn: 'Runner-Up (By PPA on 11-11-25)',
          actualRunnerUpColumn: 'Actual Runner-Up (after results on 14-11-25)',
          predictedVotesColumn: 'Expected Votes (By PPA on 11-11-25)',
          actualVotesColumn: 'Actual Votes (after results on 14-11-25)',
          predictedRunnerUpVotesColumn: 'Runner-Up Votes (By PPA on 11-11-25)',
          actualRunnerUpVotesColumn: 'Actual Runner-Up Votes (after results on 14-11-25)',
          predictedMarginColumn: 'Winning Margin (By PPA on 11-11-25)',
          actualMarginColumn: 'Actual Margin (after results on 14-11-25)',
        },
      },
    ],
  },
  bmc: {
    stateName: 'BMC (Mumbai)',
    availableElections: [
      {
        name: 'Brihanmumbai Municipal Corporation Election 2026',
        type: ElectionType.Municipal,
        electionDate: '15-01-2026',
        estimatedColumn: 'Estimated Seats (Party)',
        actualColumn: 'Actual Seats After Results on 16-01-2026',
        mergeColumns: ['Alliance', 'Expected Total Seats (Alliance)', 'Variation(+/-) 16/01/2026'],
        partyNameColumns: [
          'Party Name',
          'Expected Winning Party (By PPA on 15-01-26)',
          'Actual Winner (after results on 16-01-26)',
          'Runner-Up (By PPA on 15-01-26)',
          'Actual Runner-Up (after results on 16-01-26)',
        ],
        totalConfig: {
          title: {
            header: 'Alliance',
            colSpan: 2,
          },
          columns: {
            startHeader: 'Estimated Seats (Party)',
            colSpan: 2,
            value: 227,
          },
        },
        constituencyModalConfig: {
          constituencyColumn: 'Ward Number',
          predictedWinnerColumn: 'Expected Winning Party (By PPA on 15-01-26)',
          actualWinnerColumn: 'Actual Winner (after results on 16-01-26)',
          predictedRunnerUpColumn: 'Runner-Up (By PPA on 15-01-26)',
          actualRunnerUpColumn: 'Actual Runner-Up (after results on 16-01-26)',
          predictedVotesColumn: 'Expected Votes (By PPA on 15-01-26)',
          actualVotesColumn: 'Actual Votes (after results on 16-01-26)',
          predictedMarginColumn: 'Winning Margin (By PPA on 15-01-26)',
          actualMarginColumn: 'Actual Margin (after results on 16-01-26)',
        },
      },
    ],
  },
  tamilnadu: {
    stateName: 'Tamil Nadu',
    availableElections: [
      {
        name: 'Tamil Nadu Assembly Election 2026',
        type: ElectionType.Assembly,
        surveyDate: '29-04-2026',
        estimatedColumn: 'Estimated Seats Prediction',
        actualColumn: 'Actual Seats After Results',
        mergeColumns: ['Alliance', 'Expected Total Seats', 'District'],
        partyNameColumns: [
          'Party Name',
          'Expected Winning Party (By PPA on 29-04-26)',
          'Runner-Up (By PPA on 29-04-26)',
        ],
        isOngoing: true,
        totalConfig: {
          title: { header: 'Alliance', colSpan: 2 },
          columns: { startHeader: 'Estimated Seats Prediction', colSpan: 1, value: 234 },
        },
      },
    ],
  },
  puducherry: {
    stateName: 'Puducherry',
    availableElections: [
      {
        name: 'Puducherry Assembly Election 2026',
        type: ElectionType.Assembly,
        surveyDate: '29-04-2026',
        estimatedColumn: 'Estimated Seats Prediction',
        actualColumn: 'Actual Seats After Results',
        mergeColumns: ['Alliance', 'District', 'Expected Total Seats'],
        partyNameColumns: [
          'Party Name',
          'Expected Winning Party (By PPA on 29-04-26)',
          'Runner-Up (By PPA on 29-04-26)',
        ],
        isOngoing: true,
      },
    ],
  },
  westbengal: {
    stateName: 'West Bengal',
    availableElections: [
      {
        name: 'West Bengal Assembly Election 2026',
        type: ElectionType.Assembly,
        surveyDate: '29-04-2026',
        estimatedColumn: 'Estimated Seats Prediction',
        actualColumn: 'Actual Seats After Results',
        mergeColumns: ['District', 'Expected Total Seats'],
        partyNameColumns: ['Party Name', 'Expected Winning Party (By PPA on 29-04-26)'],
        isOngoing: true,
      },
    ],
  },
};

export const stateStats: Record<string, StateStats> = {
  andhrapradesh: {
    name: 'Andhra Pradesh',
    mlas: 175,
    mpsLokSabha: 25,
    ulbs: 125,
    rajyaSabha: 11,
    legislativeCouncil: 58,
  },
  arunachalpradesh: {
    name: 'Arunachal Pradesh',
    mlas: 60,
    mpsLokSabha: 2,
    ulbs: 35,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  assam: {
    name: 'Assam',
    mlas: 126,
    mpsLokSabha: 14,
    ulbs: 102,
    rajyaSabha: 7,
    legislativeCouncil: 0,
  },
  bihar: {
    name: 'Bihar',
    mlas: 243,
    mpsLokSabha: 40,
    ulbs: 252,
    rajyaSabha: 16,
    legislativeCouncil: 75,
  },
  chhattisgarh: {
    name: 'Chhattisgarh',
    mlas: 90,
    mpsLokSabha: 11,
    ulbs: 170,
    rajyaSabha: 5,
    legislativeCouncil: 0,
  },
  goa: {
    name: 'Goa',
    mlas: 40,
    mpsLokSabha: 2,
    ulbs: 14,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  gujarat: {
    name: 'Gujarat',
    mlas: 182,
    mpsLokSabha: 26,
    ulbs: 166,
    rajyaSabha: 11,
    legislativeCouncil: 0,
  },
  haryana: {
    name: 'Haryana',
    mlas: 90,
    mpsLokSabha: 10,
    ulbs: 92,
    rajyaSabha: 5,
    legislativeCouncil: 0,
  },
  himachalpradesh: {
    name: 'Himachal Pradesh',
    mlas: 68,
    mpsLokSabha: 4,
    ulbs: 68,
    rajyaSabha: 3,
    legislativeCouncil: 0,
  },
  jharkhand: {
    name: 'Jharkhand',
    mlas: 81,
    mpsLokSabha: 14,
    ulbs: 52,
    rajyaSabha: 6,
    legislativeCouncil: 0,
  },
  karnataka: {
    name: 'Karnataka',
    mlas: 224,
    mpsLokSabha: 28,
    ulbs: 313,
    rajyaSabha: 12,
    legislativeCouncil: 75,
  },
  kerala: {
    name: 'Kerala',
    mlas: 140,
    mpsLokSabha: 20,
    ulbs: 96,
    rajyaSabha: 9,
    legislativeCouncil: 0,
  },
  ladakh: {
    name: 'Ladakh',
    mlas: 1,
    mpsLokSabha: 1,
    ulbs: 0,
    rajyaSabha: 0,
    legislativeCouncil: 0,
  },
  madhyapradesh: {
    name: 'Madhya Pradesh',
    mlas: 230,
    mpsLokSabha: 29,
    ulbs: 414,
    rajyaSabha: 11,
    legislativeCouncil: 0,
  },
  maharashtra: {
    name: 'Maharashtra',
    mlas: 288,
    mpsLokSabha: 48,
    ulbs: 407,
    rajyaSabha: 19,
    legislativeCouncil: 78,
  },
  manipur: {
    name: 'Manipur',
    mlas: 60,
    mpsLokSabha: 2,
    ulbs: 27,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  meghalaya: {
    name: 'Meghalaya',
    mlas: 60,
    mpsLokSabha: 2,
    ulbs: 12,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  mizoram: {
    name: 'Mizoram',
    mlas: 40,
    mpsLokSabha: 1,
    ulbs: 23,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  nagaland: {
    name: 'Nagaland',
    mlas: 60,
    mpsLokSabha: 1,
    ulbs: 39,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  odisha: {
    name: 'Odisha',
    mlas: 147,
    mpsLokSabha: 21,
    ulbs: 114,
    rajyaSabha: 10,
    legislativeCouncil: 0,
  },
  punjab: {
    name: 'Punjab',
    mlas: 117,
    mpsLokSabha: 13,
    ulbs: 169,
    rajyaSabha: 7,
    legislativeCouncil: 0,
  },
  rajasthan: {
    name: 'Rajasthan',
    mlas: 200,
    mpsLokSabha: 25,
    ulbs: 230,
    rajyaSabha: 10,
    legislativeCouncil: 0,
  },
  sikkim: {
    name: 'Sikkim',
    mlas: 32,
    mpsLokSabha: 1,
    ulbs: 7,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  tamilnadu: {
    name: 'Tamil Nadu',
    mlas: 234,
    mpsLokSabha: 39,
    ulbs: 661,
    rajyaSabha: 18,
    legislativeCouncil: 0,
  },
  telangana: {
    name: 'Telangana',
    mlas: 119,
    mpsLokSabha: 17,
    ulbs: 143,
    rajyaSabha: 7,
    legislativeCouncil: 40,
  },
  tripura: {
    name: 'Tripura',
    mlas: 60,
    mpsLokSabha: 2,
    ulbs: 20,
    rajyaSabha: 1,
    legislativeCouncil: 0,
  },
  uttarpradesh: {
    name: 'Uttar Pradesh',
    mlas: 403,
    mpsLokSabha: 80,
    ulbs: 763,
    rajyaSabha: 31,
    legislativeCouncil: 100,
  },
  uttarakhand: {
    name: 'Uttarakhand',
    mlas: 70,
    mpsLokSabha: 5,
    ulbs: 112,
    rajyaSabha: 3,
    legislativeCouncil: 0,
  },
  westbengal: {
    name: 'West Bengal',
    mlas: 294,
    mpsLokSabha: 42,
    ulbs: 130,
    rajyaSabha: 16,
    legislativeCouncil: 0,
  },
  delhi: {
    name: 'Delhi',
    mlas: 70,
    mpsLokSabha: 7,
    ulbs: 5,
    rajyaSabha: 3,
    legislativeCouncil: 0,
  },
  jammukashmir: {
    name: 'Jammu & Kashmir',
    mlas: 90,
    mpsLokSabha: 5,
    ulbs: 0,
    rajyaSabha: 0,
    legislativeCouncil: 0,
  },
};

export const partyColorMapping: Record<string, StateColors> = {
  BJP: { bg: '#EC6842', fg: '#FFFFFF', border: '#C4532F' },
  INC: { bg: '#5B7FC2', fg: '#FFFFFF', border: '#4A68A0' },
  Congress: { bg: '#2563eb', fg: '#FFFFFF', border: '#1d4ed8' },
  AAP: { bg: '#00AEEF', fg: '#FFFFFF', border: '#008BC0' },
  TMC: { bg: '#FF69B4', fg: '#FFFFFF', border: '#D84D94' },
  SP: { bg: '#059669', fg: '#FFFFFF', border: '#047857' },
  BSP: { bg: '#4f46e5', fg: '#FFFFFF', border: '#3730a3' },
  YSRCP: { bg: '#059669', fg: '#FFFFFF', border: '#047857' },
  TDP: { bg: '#d97706', fg: '#000000', border: '#b45309' },
  JDU: { bg: '#4CAF50', fg: '#FFFFFF', border: '#3D8B40' },
  'LJP(RV)': { bg: '#7E57C2', fg: '#FFFFFF', border: '#5E3F9E' },
  LJP: { bg: '#7E57C2', fg: '#FFFFFF', border: '#5E3F9E' },
  RJD: { bg: '#5B9BD5', fg: '#FFFFFF', border: '#4A7CAB' },
  RLM: { bg: '#E91E8E', fg: '#FFFFFF', border: '#B81570' },
  HAMS: { bg: '#E8973D', fg: '#FFFFFF', border: '#C07A2E' },
  VIP: { bg: '#8B5CF6', fg: '#FFFFFF', border: '#6D47C5' },
  'CPI(M)': { bg: '#8B5CF6', fg: '#FFFFFF', border: '#6D47C5' },
  CPI: { bg: '#4CAF50', fg: '#FFFFFF', border: '#3D8B40' },
  'CPI(ML)': { bg: '#26A69A', fg: '#FFFFFF', border: '#1D857A' },
  AIMIM: { bg: '#C0392B', fg: '#FFFFFF', border: '#962D22' },
  Others: { bg: '#8b5cf6', fg: '#FFFFFF', border: '#6d28d9' },
  'AP-ALLIANCE': { bg: '#FDEE00', fg: '#000000', border: '#CCCC00' },
  'Shiv Sena Shinde': { bg: '#F57C00', fg: '#FFFFFF', border: '#E65100' },
  'Shiv Sena - UBT': { bg: '#EC4899', fg: '#FFFFFF', border: '#BE185D' },
  UBT: { bg: '#EC4899', fg: '#FFFFFF', border: '#BE185D' },
  'NCP - Ajit Pawar': { bg: '#10B981', fg: '#FFFFFF', border: '#059669' },
  'NCP - Sharad Pawar': { bg: '#22C55E', fg: '#FFFFFF', border: '#16A34A' },
  MNS: { bg: '#F59E0B', fg: '#000000', border: '#D97706' },
  VBA: { bg: '#4169E1', fg: '#FFFFFF', border: '#2E5CB8' },
  IND: { bg: '#EF4444', fg: '#FFFFFF', border: '#DC2626' },
  'IND/OTHERS': { bg: '#EF4444', fg: '#FFFFFF', border: '#DC2626' },
  DMK: { bg: '#E60000', fg: '#FFFFFF', border: '#B30000' },
  AIADMK: { bg: '#1B7F1B', fg: '#FFFFFF', border: '#145C14' },
  DMDK: { bg: '#FBBF24', fg: '#000000', border: '#D49B0F' },
  VCK: { bg: '#1E40AF', fg: '#FFFFFF', border: '#1E3A8A' },
  PMK: { bg: '#FACC15', fg: '#000000', border: '#CA8A04' },
  AMMK: { bg: '#7C3AED', fg: '#FFFFFF', border: '#5B21B6' },
  IUML: { bg: '#16A34A', fg: '#FFFFFF', border: '#15803D' },
  TVK: { bg: '#DB2777', fg: '#FFFFFF', border: '#9D174D' },
  AITC: { bg: '#FF69B4', fg: '#FFFFFF', border: '#D84D94' },
  GJM: { bg: '#15803D', fg: '#FFFFFF', border: '#166534' },
  AINRC: { bg: '#F26C21', fg: '#FFFFFF', border: '#C25217' },
  LJK: { bg: '#9333EA', fg: '#FFFFFF', border: '#6B21A8' },
};
