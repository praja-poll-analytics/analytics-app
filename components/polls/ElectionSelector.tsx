...
import { electionTypeLabels } from './data';
import { ElectionConfig, ElectionType } from './types';

const electionTypeBadgeClasses: Record<ElectionType, string> = {
  [ElectionType.Assembly]: 'bg-blue-100 text-blue-800',
  [ElectionType.LokSabha]: 'bg-green-100 text-green-800',
  [ElectionType.Municipal]: 'bg-orange-100 text-orange-800',
  [ElectionType.ByElection]: 'bg-amber-100 text-amber-800',
};
              ...
              <div className="flex items-center justify-between">
                <span
                  className={`text-sm font-medium px-3 py-1 rounded-full ${
                    electionTypeBadgeClasses[election.type]
                  }`}
                >
                  {electionTypeLabels[election.type]}
                </span>
                {currentElection.name === election.name && (
