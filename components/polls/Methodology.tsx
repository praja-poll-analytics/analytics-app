...
const bmcEnglishItems = [
  'Based on field visit to wards and contacting the residents of the area.',
  'Teams visited wards with survey sheets and questionnaire both structured and unstructured',
  'Consisted of participative and non-participative observation of residents responses.',
  'Samples of 1000 to 2000 have been collected of different age groups ,different social back grounds,professionals,male and female,students,auto rickshaw drivers,labourers,and other segments of society.',
];

const mapping: { [key: string]: { english: string[]; hindi: string[] } } = {
  bihar: {
    english: biharEnglishItems,
    hindi: biharHindiItems,
  },
  bmc: {
    english: bmcEnglishItems,
    hindi: [],
  },
};

export const Methodology = ({ stateId }: { stateId: string }) => {
  const englishItems = mapping[stateId]?.english || [];
  const hindiItems = mapping[stateId]?.hindi || [];
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Methodology{hindiItems.length > 0 ? ' / कार्यप्रणाली' : ''}</h1>
